...
function AffichageEleve() {
    ...
    const categories = useMemo(() => {
        const uniqueclasse = [...new Set(classe.map(p => p.nomniveau))];
        return ['Tous', ...uniqueclasse];
    }, [classe]);
            ...
            <div className=' shadow-xl rounded-xl p-6 mb-8 border transition duration-300'>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100/50">
                    <div className="flex flex-wrap gap-2">
                        <FaSchool className={`w-5 h-5 mr-1 text-indigo-600`} />
                        <span className={`font-semibold mr-3 hidden sm:inline`}>Filtre par Classe:</span>
                        {categories.map(key => (
                            <button 
                                key={key} 
                                    ...
                                    <tr key={idx}>
                                        ...
                                        <td>
                                            {liste.inscription?.personne?.photo ? (
                                                ...
                                                />
                                            ) : <span className="text-gray-400 text-center text-sm">Aucune</span>}
                                        </td>
                                        ...
                                        <td className="px-2 py-2 text-center flex justify-center gap-1">
                                            <button 
                                                onClick={() => openCarteModal(liste)} 
                                                className="flex items-center h-8 text-white bg-blue-600 p-1 rounded text-xs"
                                            >
                    ...
                    <div key={idx} className={`rounded-xl shadow-lg p-4 flex flex-col gap-3 border transition duration-300`}>
                        ...
                        <div className="flex items-center justify-between border-b pb-3 border-gray-200/50">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => openCarteModal(liste)} title="Carte"
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                            ...
                            <DetailMobile label="Date de Naissance" value={liste.inscription?.personne?.naiss} />
                            <DetailMobile label="Lieu de Naissance" value={liste.inscription?.personne?.lieunaiss} />
