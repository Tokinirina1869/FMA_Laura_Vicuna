    ...
    <div className="p-4 sm:p-8 min-h-screen">
      ...
      <div className={`shadow-xl rounded-xl p-6 mb-8 border transition duration-300`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-end mb-6">
          ...
          <div className="lg:col-span-1 flex gap-3">
            ...
            <button onClick={resetFilters} 
                className="flex items-center px-4 py-2 bg-green-600 text-white rounded-lg shadow font-semibold hover:bg-green-700 transition">
                <FaSync className="mx-2 w-5 h-5" /> Refraichir
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100/50">
          ...
          <div className="flex flex-wrap gap-2">
            <FaPen className={`w-5 h-5 mr-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`} />
            <span className={`font-semibold mr-3 hidden sm:inline `}>Filtre Formation:</span>
            {loadingCategories ? (
              <div className="flex items-center gap-2">
                <RefreshCw className="w-4 h-4 animate-spin" />
                <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>Chargement des formations...</span>
              </div>
            ) : (
              categories.map(key => (
                ...
                {currentData.length > 0 ? (
                  currentData.map((liste, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-2 py-2 text-center text-sm">{liste.inscription?.personne?.cin || 'Mineur'}</td>
                      ...
                      <td className="px-2 py-2 text-center flex justify-center gap-1">
                        <button onClick={openDetailsModal(liste)} className="flex items-center h-8 text-white bg-green-600 p-1 rounded text-xs hover:bg-green-700">
                          <FaEye className="mx-1 w-3 h-3"/> Détails
                        </button>
                        <button onClick={openModalCarte(liste)} className="flex items-center h-8 text-white bg-yellow-600 p-1 rounded text-xs hover:bg-yellow-700">
                          <FaIdCard className="mx-1 w-3 h-3" /> Carte
                        </button>
                        <button onClick={openModal(liste)} className="flex items-center text-white bg-blue-600 p-1 rounded text-xs hover:bg-blue-700">
                          <FaEdit className="mx-1 w-3 h-3"/> Modifier
                        </button>
                        <button onClick={() => handleDelete(liste.inscription?.personne?.matricule)} className="flex items-center text-white bg-red-600 p-1 rounded text-xs hover:bg-red-700">
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center">
                      ...
                      <p className={isDark ? 'text-gray-300' : 'text-gray-600'}>Aucune donnée trouvée</p>
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <div className="md:hidden flex flex-col gap-4 mt-6">
        {currentData.length > 0 ? currentData.map((liste, idx) => (
          <div key={idx} className={`rounded-xl shadow-lg p-4 flex flex-col gap-3 border transition duration-300 `}>
            ...
            <div className="flex items-center justify-between border-b pb-3 border-gray-200/50">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={openModalCarte(liste)} title="Créer carte"
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <DetailMobile label="Matricule" value={liste.inscription?.personne?.matricule} isDark={isDark} />
              <DetailMobile label="CIN" value={liste.inscription?.personne?.cin} isDark={isDark} />
              <DetailMobile label="Date Naiss." value={liste.inscription?.personne?.naiss} isDark={isDark} />
              <DetailMobile label="Lieu Naiss." value={liste.inscription?.personne?.lieunaiss} isDark={isDark} />
            </div>
          </div>
        )) : (
