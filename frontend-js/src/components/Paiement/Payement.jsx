...
function PaymentPage() {
  ...
  const isLyceeMatricule = (matricule) => {
    return matricule && matricule.includes('/LYC/')
  };
  const isCfpMatricule = (matricule) => {
    return matricule && !matricule.includes('/LYC/')
  };


  // Après le chargement de listeInsc, définissez une variable filtrée selon le rôle
  const filteredInsc = useMemo(() => {
    if (!listeInsc.length) return [];
    if (user?.role === 'secretaire_lycee') {
      return listeInsc.filter(i => isLyceeMatricule(i.matricule));
    } else if (user?.role === 'secretaire_cfp') {
      return listeInsc.filter(i => isCfpMatricule(i.matricule));
    }
    return listeInsc;
  }, [listeInsc, user]);
  ...
  const generateReceipt = async (paiement) => {
    try {
      ...
      doc.text(`Total Payé : ${safeNumber(paiement.montantpaie).toLocaleString()} Ar`, pageWidth - 45, afterTableY + 10, { align: "right" });

      doc.setFont("helvetica", "normal"); doc.setFontSize(12);
      doc.text(`Reste à payer : ${safeNumber(reste).toLocaleString()} Ar`, pageWidth - 45, afterTableY + 18, { align: "right" });
  ...
  const generateDailyReceiptsPDF = async () => {
    try {
      ...
      try {
        if (window.showSaveFilePicker) {
          const fileHandle = await window.showSaveFilePicker({
            suggestedName: `Reçus_Aujourd'hui_${todayStr}.pdf`,
            types: [
              {
                description: "Fichiers PDF",
                accept: { "application/pdf": [".pdf"] },
  ...
  const filterPaiements = useMemo(() => {
    ...
    if (user?.role === 'secretaire_lycee') {
      filtered = filtered.filter(p => isLyceeMatricule(p.matricule));
    } else if (user?.role === 'secretaire_cfp') {
      filtered = filtered.filter(p => isCfpMatricule(p.matricule));
    }
                          ...
                          {currentData.length > 0 ? (
                            currentData.map((liste, index) => (
                              ...
                              >
                                ...
                                <td className={`text-center px-2 py-2 font-bold ${calculateResteSafe(liste) > 0 ? 'text-red-500' : 'text-green-500'
                                  }`}>
                                  ...
                                  <div className="grid grid-cols-2 gap-1 min-w-[120px]">
                                    {[
                                      ...
                                      <motion.button
                                        ...
                                        whileTap={{ scale: 0.95 }}
                                        className={`flex items-center justify-center text-white px-3 py-2 rounded text-xs font-medium w-full ${btn.color === 'indigo' ? 'bg-indigo-600 hover:bg-indigo-700' :
